...
function Dashboard({
  ...
  const sellerTarget = userTarget
    ? listSellers.find((item) => item.id === userTarget.sellerId)
    : null;
  ...
  const productsTarget = sellerTarget.productIds
    .map((pid) => listProducts.find((p) => p.id === pid))
    .filter(Boolean);

  const ordersTarget = listOrders.filter((o) => o.sellerId === sellerTarget.id);
          ...
          >
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h4 className="text-3xl font-bold text-gray-900 mb-1">
                  {calculateTotalEarnings(ordersTarget)}{" "}
                  <span className="text-sm font-medium text-gray-400">MAD</span>
                </h4>
                <p className="text-sm text-gray-500 flex items-center gap-1">
                  {dashboard.total_earnings}
                  <span className="text-xs bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full ml-2 font-medium">
                    {dashboard.month}
                  </span>
                </p>
              </div>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h4 className="text-3xl font-bold text-gray-900 mb-1">
                  {calculateOrdersPending(ordersTarget)}
                </h4>
                <p className="text-sm text-gray-500">
                  {dashboard.orders_pending}
                </p>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <h4 className="text-3xl font-bold text-gray-900 mb-1">
                  {sellerTarget.subscription.type} Plan
                </h4>
                <p className="text-sm text-gray-500">
                  {dashboard.active_plan}{" "}
                  <span className="font-medium text-gray-700">
                    {formatShortDate(sellerTarget.subscription.endDate)}
                  </span>
                </p>
              </div>
            </motion.div>

...
function pricing({ pricing, setAddSeller }) {
  const plans = [
    {
    ...
    },
    {
      ...
      notIncluded: ["Dedicated Ad Budget", "Professional Product Photography"],
    },
    {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto items-start">
          {plans.map((plan, index) => (
            <Reveal key={plan.name} delay={index * 0.2} className="h-full">
              <div
                className={`relative bg-white rounded-3xl shadow-xl flex flex-col transition-transform duration-300 hover:-translate-y-2 ${
                  plan.isPopular
                    ? "border-2 border-emerald-500 scale-105 z-10 shadow-2xl"
              ...
              >
                ...
                <div className="p-8 bg-gray-50 rounded-b-3xl border-t border-gray-100">
                  <Link
                    ...
                    to="/register"
                    className={`w-full block text-center py-4 rounded-xl font-bold transition-all duration-300 shadow-md hover:shadow-lg ${
                      plan.isPopular
